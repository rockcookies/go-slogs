...
func TestLogger_WithAttrs(t *testing.T) {
	buf := &bytes.Buffer{}
	h := slogs.NewHandler(slog.NewJSONHandler(buf, nil))
	logger := slogs.New(h).With("app", "test")
...
func TestLogger_WithGroup(t *testing.T) {
	buf := &bytes.Buffer{}
	h := slogs.NewHandler(slog.NewJSONHandler(buf, nil))
	logger := slogs.New(h).WithGroup("group1")
...
func TestLogger_Enabled(t *testing.T) {
	buf := &bytes.Buffer{}
	h := slogs.NewHandler(slog.NewJSONHandler(buf, &slog.HandlerOptions{Level: slog.LevelWarn}))
	logger := slogs.New(h)
